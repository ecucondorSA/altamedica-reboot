...
export function getAppUrl(path: string, subdomain?: string): string {
  const baseUrl = ensureEnv('NEXT_PUBLIC_APP_URL') ?? 'http://localhost:3000';
  ...
  if (subdomain) {
    const productionUrls = {
      patients: ensureEnv('NEXT_PUBLIC_PATIENTS_URL') ?? 'https://patients.autamedica.com',
      doctors: ensureEnv('NEXT_PUBLIC_DOCTORS_URL') ?? 'https://doctors.autamedica.com',
      companies: ensureEnv('NEXT_PUBLIC_COMPANIES_URL') ?? 'https://companies.autamedica.com',
      admin: ensureEnv('NEXT_PUBLIC_ADMIN_URL') ?? 'https://admin.autamedica.com'
    };
...
export function getApiUrl(path: string): string {
  const apiUrl = ensureEnv('NEXT_PUBLIC_API_URL') ?? 'http://localhost:3000/api';
  return `${apiUrl}${path}`;
}