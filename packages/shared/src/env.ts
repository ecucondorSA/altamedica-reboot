...
export function ensureClientEnv(name: string): string {
  if (!ALLOWED_CLIENT_VARS.has(name)) {
    throw new Error(`Invalid client environment variable: ${name}. Only NEXT_PUBLIC_* variables are allowed on client side.`);
  }
...
export function validateEnvironmentSecurity(): void {
  ...
  if (process.env.NEXT_PUBLIC_OPENAI_API_KEY && !process.env.ALLOW_CLIENT_OPENAI_KEY) {
